...
export class NullBrowser implements Browser {
  ...
  >(
    eventName: K,
    handler: (
      e: import('puppeteer').BrowserEventObj[K],
      ...args: any[]
    ) => void,
  ): this {
  ...
  waitForTarget(
    predicate: (target: import('puppeteer').Target) => boolean,
    options?: import('puppeteer').Timeoutable | undefined,
  ): Promise<import('puppeteer').Target> {
    throw new Error('Browser not initialized');
  }
}
